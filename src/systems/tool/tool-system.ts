...
export class ToolSystem {
  ...
  /** Shared select attempt for both digit-key and wheel input — enforces
   * spec三's "捕貨鉤必須空手使用...若玩家正在持有任何物品：不可切換至捕貨
   * 鉤" (Pallet/dolly holds also set state away from 'empty-handed', so this
   * one check covers every kind of "currently holding something" case, not
   * just PickupSystem's own heldObjectId). */
  private trySelect(tool: ActiveTool): void {
