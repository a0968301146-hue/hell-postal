...
import { HUD } from '../hud';
import { TUTORIAL_ENTRIES, TutorialEntry } from '../../data/tutorial-data';
import {
  ACTION_ORDER, ACTION_LABELS, InputAction, UNWIRED_ACTIONS,
} from '../../adapters/browser-input/input-binding-manager';
import { RegionCodexId } from '../../data/world/region-codex-data';
import { StampRegionId } from '../../data/world/stamp-collection-data';
import { CodexUI, CodexTab } from './codex-ui';
...
export class ManualUI {
  ...
  /** Stateless render helper for the 圖鑑 bookmark — see codex-ui.ts's own
   * class doc comment for why it owns none of the selection/tab state
   * itself. */
  ...
  constructor(
    ...
    this.codexUI = new CodexUI(settingsManager, this.getCurrentDay);
  ...
  private renderCodex(): void {
    this.codexUI.render(this.leftPageEl, this.rightPageEl, this.codexTab, {
      vehicleId: this.selectedVehicleId,
      speciesId: this.selectedSpeciesId,
      cargoId: this.selectedCargoId,
      regionId: this.selectedRegionId,
      stampRegionId: this.selectedStampRegionId,
    });
  }
}
